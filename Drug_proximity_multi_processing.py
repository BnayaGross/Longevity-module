...
path_all_drugbank_drugs = "./data/all_drugbank_drugs.csv"
...
print("Start measuring proximity using multiprocessing")

manager = multiprocessing.Manager()
shared_dataframe = manager.list()

# Define the number of processes you want to use
num_processes = int(sys.argv[1])
level = int(sys.argv[2])
print("num_processes", num_processes, "Level: ", level)
...
for am in amg_list:
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        # Use pool.starmap to execute the function in parallel with both inputs
        results = pool.starmap(prox_adapter, [(G_sub,target_genes_bucket[idx],longevity_genes,D,approved_drugs_names[idx],  am) for idx in range(len(approved_drugs_names))])

        # Append the results to the shared dataframe
        shared_dataframe.extend(results)

compunds_pval = pd.DataFrame()
for new_row in list(shared_dataframe):
     compunds_pval = compunds_pval.append(new_row, ignore_index=True)
        

compunds_pval.to_csv('Proximity_drugs_log_binning_level' + str(level) + '.csv', index=False) 
